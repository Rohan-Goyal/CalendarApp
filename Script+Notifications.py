# WARNING! The Cronofy Calendar Integration will NOT work as it stands, until you set a proper ID/ access token,
#  so don't freak out if it doesn't work
...
crono_cal = Client(access_token="Fill in here")
...
if platform.system() == "Darwin":
    ...
    def create_session(session, cronosession):
        print(session, cronosession)
...
class Ui_MainWindow(QtWidgets.QMainWindow):  # Most of this is autogenerated UI setup
    ...
    def write_session(self, time,
        ...
        if free_times_filtered: #TODO: refine this test
            ...
            for timeframe in free_times_filtered:
                if timeframe['start'] <= time <= timeframe['end'] :
                    ...
                    create_session(session=memory_dict,
                        cronosession={
                            'event_id':random_id,
                            'sessionplan': memory_dict['notes'],
                            'description': 'NA',
                            'start': time,
        ...
        constructed_date =(datetime.datetime.now().date() + datetime.timedelta(days=displacement)).astimezone(
            pytz.utc).isoformat()
...
class FormWindow(QtWidgets.QMainWindow):
    ...
    def save_event(self):
        ...
        cronofy_dict = {
            ...
            'start': iso_date_utc,
            'end':(datetime.datetime.fromisoformat(iso_date) + datetime.timedelta(hours=2)).isoformat()
